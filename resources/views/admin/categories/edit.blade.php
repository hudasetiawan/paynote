                        ...
                        <form action="{{ route('categories.update', $category->id_category) }}" method="POST">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <select class="form-control @error('type') is-invalid @enderror" id="type"
                                    name="type">
                                    <option value="income" {{ $category->type == 'income' ? 'selected' : '' }}>Pemasukan
                                    </option>
                                    <option value="expense" {{ $category->type == 'expense' ? 'selected' : '' }}>Pengeluaran
                                    </option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <a href="{{ route('categories') }}" class="btn btn-sm btn-secondary mt-3">Kembali</a>
                        </form>
                    </div>
                </div>
            </div>
